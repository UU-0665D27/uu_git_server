...
- 📁 **Модель репозиториев**: путь вида `owner/repo` (ровно два сегмента). Репозитории создаются автоматически как bare при первом обращении.
- 🚫 **Контроль доступа на запись**: `git push` разрешён только владельцу репозитория (сегмент `owner` в пути должен совпадать с именем аутентифицированного пользователя). Чтение (`fetch`/`clone`) доступно всем, включая анонимного пользователя `public` по SSH.
...
- **Авторизация на запись**: сравнивается первый сегмент пути (`owner`) с именем аутентифицированного пользователя; при несовпадении `push` отклоняется (403 по HTTP, `channel_failure` по SSH).
